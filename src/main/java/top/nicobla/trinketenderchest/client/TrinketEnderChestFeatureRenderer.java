...
public class TrinketEnderChestFeatureRenderer<T extends LivingEntity, M extends EntityModel<T>> extends FeatureRenderer<T, M> {

  private final ShulkerEntityModel<?> model;

  public TrinketEnderChestFeatureRenderer(FeatureRendererContext<T, M> ctx, EntityModelLoader loader) {
    super(ctx);
    ...
    TrinketsApi.getTrinketComponent(entity).ifPresent(component -> {
      ...
      for (Map.Entry<String, Map<String, TrinketInventory>> group : component.getInventory().entrySet()) {
        ...
        for (Map.Entry<String, TrinketInventory> slotType : group.getValue().entrySet()) {
          ...
          //   TrinketEnderChestMod.TRINKET_ENDER_CHEST_COMPONENT.maybeGet(stack).ifPresent(trinket -> {
          //     Direction direction = Direction.SOUTH;
          //     SpriteIdentifier spriteIdentifier = TexturedRenderLayers.SHULKER_TEXTURE_ID; // ENDER
          ...
          //       matrices.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(bipedEntityModel.body.pitch * TrinketRenderer.MAGIC_ROTATION));
          //     }
          //     matrices.push();
          ...
          //     modelPart.setPivot(0.0F, 24.0F - trinket.getAnimationProgress(tickDelta) * 0.5F * 16.0F, 0.0F);
